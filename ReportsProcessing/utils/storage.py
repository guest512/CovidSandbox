...
def get_country_regions(country_name: str) -> _List[str]:
    ...
    return list(
        map(lambda file_name: _os.path.splitext(file_name)[0],
            _os.listdir(__get_country_regions_folder(country_name))))
...
def get_country_report(country_name: str,
                       parse_dates=True,
    ...
    report_file = _os.path.join(__reports_path, country_name,
                                country_name + ".csv")
    country_df = None
...
def get_region_report(country_name: str,
                      region_name: str,
                      parse_dates=True,
                      date_is_index=True) -> _pd.DataFrame:
    ...
    '''
    report_file = _os.path.join(__reports_path, country_name, "regions",
                                region_name + ".csv")
    region_df = None
